...
const TERMS = [
  ...
  { id: 'term3', label: 'الترم الثالث' },
];

const MOCK_DATA = {
  term1: {
    lectures: [
      { id: 1, title: 'مقدمة في التجويد', date: '2024-09-01', hasQuiz: true, quizScore: 85, hasAssignment: true, assignmentStatus: 'submitted' },
      { id: 2, title: 'أحكام النون الساكنة', date: '2024-09-08', hasQuiz: true, quizScore: null, hasAssignment: true, assignmentStatus: 'late' },
      { id: 3, title: 'أحكام الميم الساكنة', date: '2024-09-15', hasQuiz: false, quizScore: null, hasAssignment: false, assignmentStatus: null },
    ],
    attendance: [
      { date: '2024-09-01', status: 'present', checkIn: '08:00', checkOut: '12:00' },
      { date: '2024-09-08', status: 'present', checkIn: '08:15', checkOut: '12:00' },
      { date: '2024-09-15', status: 'absent', checkIn: null, checkOut: null },
    ]
  },
  term2: {
    lectures: [
      { id: 4, title: 'المدود - الجزء الأول', date: '2025-01-05', hasQuiz: true, quizScore: 90, hasAssignment: true, assignmentStatus: 'grading' },
    ],
    attendance: [
      { date: '2025-01-05', status: 'present', checkIn: '08:00', checkOut: '12:00' },
    ]
  },
  term3: {
    lectures: [],
...
export default function StudentDashboard() {
  ...
  // Load from LocalStorage (Optional requirement)
  useEffect(() => {
    const savedTerm = localStorage.getItem('student-active-term');
    if (savedTerm) setActiveTerm(savedTerm);
  }, []);
  ...
  const currentData = MOCK_DATA[activeTerm as keyof typeof MOCK_DATA];
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans" dir="rtl">
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div className="hidden md:flex items-center gap-4">
              <div className="flex items-center gap-2 px-3 py-1 bg-slate-100 rounded-full">
                ...
                <span className="text-sm font-medium text-slate-700">أحمد محمد (طالب)</span>
              </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Tabs defaultValue="term1" value={activeTerm} onValueChange={handleTermChange} className="space-y-6">
          ...
          {TERMS.map((term) => (
            <TabsContent key={term.id} value={term.id} className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <CardContent className="p-6">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <h3 className="text-3xl font-bold">{MOCK_DATA[term.id as keyof typeof MOCK_DATA].lectures.length}</h3>
                      </div>
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <h3 className="text-3xl font-bold text-slate-800">
                          {MOCK_DATA[term.id as keyof typeof MOCK_DATA].attendance.length > 0 ? '85%' : '0%'}
                        </h3>
                      </div>
                  ...
                  <CardContent className="p-6">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <h3 className="text-3xl font-bold text-slate-800">
                          {MOCK_DATA[term.id as keyof typeof MOCK_DATA].lectures.filter(l => l.assignmentStatus === 'submitted').length}
                        </h3>
                      </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                  ...
                  {MOCK_DATA[term.id as keyof typeof MOCK_DATA].lectures.length > 0 ? (
                    <div className="space-y-4">
                      {MOCK_DATA[term.id as keyof typeof MOCK_DATA].lectures.map((lecture) => (
                                  ...
                                  <div className="flex gap-2 text-xs">
                                    ...
                                    {lecture.hasAssignment && (
                                      <span className={`px-2 py-1 rounded-full ${
                                        lecture.assignmentStatus === 'submitted' ? 'bg-blue-100 text-blue-700' : 
                                        lecture.assignmentStatus === 'late' ? 'bg-red-100 text-red-700' : 
                                        'bg-yellow-100 text-yellow-700'
                                      }`}>
                                        {lecture.assignmentStatus === 'submitted' ? 'تم التسليم' : 
                                         lecture.assignmentStatus === 'late' ? 'متأخر' : 
                                         lecture.assignmentStatus === 'grading' ? 'قيد التقييم' : 'مطلوب تكليف'}
                                      </span>
                                    )}
                                  </div>
                                </div>
                              ...
                              <div className="flex flex-wrap gap-3 mt-4">
                                ...
                                {lecture.hasAssignment && (
                                  <Button variant="outline" size="sm" className="gap-2">
                                    ...
                                    {lecture.assignmentStatus ? 'إعادة الرفع' : 'رفع التكليف'}
                                  </Button>
                                )}
                              </div>
                            </div>
                  ...
                  <Card className="bg-gradient-to-r from-slate-900 to-slate-800 text-white border-none">
                    <CardContent className="p-6 flex justify-between items-center">
                      <div>
                        ...
                        <p className="text-slate-300 text-sm">اختبار نهاية {term.label}</p>
                      </div>
                  ...
                  <Card>
                    <CardContent className="p-0">
                      <div className="divide-y divide-slate-100">
                        {MOCK_DATA[term.id as keyof typeof MOCK_DATA].attendance.length > 0 ? (
                          MOCK_DATA[term.id as keyof typeof MOCK_DATA].attendance.map((record, idx) => (
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-xs text-slate-500">
                                    {record.status === 'present' ? `${record.checkIn} - ${record.checkOut}` : 'غياب'}
                                  </p>
                                </div>
                              </div>
